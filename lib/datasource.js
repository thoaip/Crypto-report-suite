...
// CAS_EXCHANGE = comma list of exchanges to try in order (fallback chain).
// Cloud/CI: set "okx,bybit,kraken,binance" since some exchanges geo-block
// datacenter IPs (Binance 451, OKX region blocks). First that responds wins.
const EX_LIST = (process.env.CAS_EXCHANGE || "binance")
  .toLowerCase()
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
const clients = {};
function client(id, futures = false) {
  const key = id + (futures ? ":f" : "");
  if (!clients[key]) {
    const opts = { enableRateLimit: true, timeout: 15000 };
    if (futures) opts.options = { defaultType: id === "okx" ? "swap" : "future" };
    clients[key] = new ccxt[id](opts);
  }
  return clients[key];
}

let preferredSpot = null; // remember the exchange that last worked

/** Try `fn(client)` across the exchange list until one succeeds. */
async function resilient(fn, futures = false) {
  const order = preferredSpot && EX_LIST.includes(preferredSpot)
    ? [preferredSpot, ...EX_LIST.filter((x) => x !== preferredSpot)]
    : EX_LIST;
  const errs = [];
  for (const id of order) {
    try {
      const r = await fn(client(id, futures));
      if (!futures) preferredSpot = id;
      return r;
    } catch (e) {
      errs.push(`${id}: ${(e.message || String(e)).slice(0, 70)}`);
    }
  }
  throw new Error(`all exchanges failed → ${errs.join(" | ")}`);
}

function spot() {
  return client(preferredSpot && EX_LIST.includes(preferredSpot) ? preferredSpot : EX_LIST[0]);
}

/** Probe a broad list of exchanges for reachability (diagnostic). */
async function probeExchanges() {
  const ids = ["kraken", "bitstamp", "coinbase", "okx", "kucoin", "gateio", "bybit", "binance"];
  const out = [];
  for (const id of ids) {
    if (!ccxt[id]) { out.push({ id, ok: false, err: "no ccxt module" }); continue; }
    try {
      const c = new ccxt[id]({ enableRateLimit: true, timeout: 8000 });
      let t;
      try { t = await c.fetchTicker("BTC/USDT"); } catch { t = await c.fetchTicker("BTC/USD"); }
      out.push({ id, ok: true, price: t.last });
    } catch (e) {
      out.push({ id, ok: false, err: (e.message || String(e)).slice(0, 60) });
    }
  }
  return out;
}
...
async function getCoinbasePremium(symbol = "BTC") {
  try {
    ...
    const [usd, usdt] = await Promise.all([
      ...
      resilient((c) => c.fetchTicker(base + "/USDT")),
    ]);
...
function futures() {
  return client(preferredSpot && EX_LIST.includes(preferredSpot) ? preferredSpot : EX_LIST[0], true);
}
...
async function getTicker(symbol) {
  ...
  const t = await resilient((c) => c.fetchTicker(sym));
  return {
    symbol: t.symbol,
    last: t.last,
    bid: t.bid,
...
async function getOHLCV(symbol, timeframe = "4h", limit = 200) {
  const sym = normalizeSymbol(symbol);
  const raw = await resilient((c) => c.fetchOHLCV(sym, timeframe, undefined, limit));
  return {
    opens: raw.map((c) => c[1]),
    highs: raw.map((c) => c[2]),
    lows: raw.map((c) => c[3]),
...
const TF_MS = { "1m": 60e3, "5m": 300e3, "15m": 900e3, "30m": 1800e3, "1h": 3600e3, "2h": 7200e3, "4h": 14400e3, "6h": 21600e3, "12h": 43200e3, "1d": 86400e3, "1w": 604800e3 };

/**
 * Paginated OHLCV — most exchanges cap a single fetch at ~300 candles. Pages
 * FORWARD from (now - total·tf) so we can pull thousands of bars for backtests.
 * Dedupes by timestamp, returns the most recent `total`.
 */
async function getOHLCVPaged(symbol, timeframe = "15m", total = 1500) {
  const sym = normalizeSymbol(symbol);
  const tfMs = TF_MS[timeframe] || 900e3;
  const now = Date.now();
  let since = now - total * tfMs;
  const map = new Map();
  let guard = 0;
  while (map.size < total + 50 && guard++ < 60) {
    const batch = await resilient((c) => c.fetchOHLCV(sym, timeframe, since, 300));
    if (!batch || !batch.length) break;
    for (const c of batch) map.set(c[0], c);
    const lastTs = batch[batch.length - 1][0];
    if (batch.length < 300 || lastTs >= now - tfMs) break;
    since = lastTs + tfMs;
  }
  const raw = [...map.values()].sort((a, b) => a[0] - b[0]).slice(-total);
  return {
    opens: raw.map((c) => c[1]), highs: raw.map((c) => c[2]), lows: raw.map((c) => c[3]),
    closes: raw.map((c) => c[4]), volumes: raw.map((c) => c[5]), raw,
  };
}
