...
const { smcAnalysis, detectSMT } = require("./smc");
const kz = require("./killzone");

const round = (n, d = 2) =>
  n == null ? null : Math.round(n * 10 ** d) / 10 ** d;

/** Compute the core indicator snapshot for one timeframe. opts → SMC (smt, hourUTC). */
function computeIndicators(o, opts = {}) {
  ...
  const smc = smcAnalysis(o.opens, highs, lows, closes, opts.smc || {});
...
async function fullAnalysis(symbol = "BTC", entryTf = "4h") {
  ...
  // SMT divergence vs a correlated asset (BTC↔ETH; alts vs BTC) on the entry tf.
  let smt = null;
  try {
    const smtPair = base === "BTC" ? "ETH" : "BTC";
    const peer = await ds.getOHLCV(smtPair, entryTf, 120);
    smt = detectSMT(entry.highs, entry.lows, peer.highs, peer.lows);
    if (smt) smt.peer = smtPair;
  } catch { smt = null; }
  const hourUTC = new Date().getUTCHours();
  const smcOpts = { smc: { smt, hourUTC } };

  // ICT Killzone signal on the 1h entry timeframe (optimized params).
  let killzoneSignal = null;
  try {
    const o1h = await ds.getOHLCV(symbol, "1h", 200);
    killzoneSignal = kz.killzoneSignal(o1h, { hourUTC, params: kz.loadParams() });
  } catch { killzoneSignal = null; }

  const dailyIx = computeIndicators(daily);
  const entryIx = computeIndicators(entry, smcOpts); // entry tf gets SMT + killzone
  const weeklyIx = computeIndicators(weekly); // 1W timeframe (for BB squeeze / converging)

  // ---- Long-term CYCLE metrics (1W MA200/MA350, weekly MACD, Vortex, Mayer) ----
  const cycle = (() => {
    const wc = weekly.closes, wn = wc.length;
    const price = wc[wn - 1];
    const sma = (arr, p) => (arr.length >= p ? arr.slice(-p).reduce((a, b) => a + b, 0) / p : null);
    const ma200w = sma(wc, 200);
    const ma350w = sma(wc, 350);
    const ma200d = sma(daily.closes, 200);
    const vortexW = ind.vortex(weekly.highs, weekly.lows, wc, 14);
    const ath = Math.max(...weekly.highs);
    const athIdx = weekly.highs.indexOf(ath);
    const cycLow = Math.min(...weekly.lows.slice(Math.max(0, athIdx - 250), athIdx + 1));
    const range = ath - cycLow;
    return {
      price,
      ma200w, ma350w, ma200d,
      mayer: ma200d ? price / ma200d : null,
      weeklyMacd: weeklyIx.macd,
      vortexW,
      ath, cycleLow: cycLow,
      retraceFromAthPct: range > 0 ? ((ath - price) / range) * 100 : null,
      fib: range > 0 ? { f236: ath - 0.236 * range, f382: ath - 0.382 * range, f50: ath - 0.5 * range } : null,
    };
  })();
  ...
  return {
    ...
    smc: {
      entry: entryIx.smc && {
        ...
        ote: entryIx.smc.ote,
        displacement: entryIx.smc.displacement,
        killzone: entryIx.smc.killzone,
        smt: smt && smt.bias !== "neutral" ? { bias: smt.bias, peer: smt.peer, note: smt.note } : null,
    ...
    cycle: {
      ma200w: round(cycle.ma200w), ma350w: round(cycle.ma350w),
      mayer: round(cycle.mayer), retraceFromAthPct: round(cycle.retraceFromAthPct),
      weeklyMacdBear: cycle.weeklyMacd ? cycle.weeklyMacd.macd < cycle.weeklyMacd.signal : null,
      vortexBull: cycle.vortexW ? cycle.vortexW.bullish : null,
      vortexWeeksSinceCross: cycle.vortexW ? cycle.vortexW.barsSinceCross : null,
      ath: round(cycle.ath), cycleLow: round(cycle.cycleLow),
      fib: cycle.fib && { f236: round(cycle.fib.f236), f382: round(cycle.fib.f382), f50: round(cycle.fib.f50) },
    },
    killzoneSignal: killzoneSignal && killzoneSignal.active ? killzoneSignal : null,
    bias: {
...
module.exports = { computeIndicators, fullAnalysis, scoreTimeframe, detectPatterns, buildSignal };
