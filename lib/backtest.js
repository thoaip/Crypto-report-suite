 ...
 */
/**
 * Walk-forward reconstruction of the council's historical forecasts → trade list.
 * Returns { trades, neutral, o, cfg, memberStats } (raw, reusable by Monte Carlo).
 */
async function generateTrades(symbol, opts = {}) {
  ...
  for (let i = warmup; i < n - horizon; i++) {
    ...
    if (!overlapping && i < nextAvailable) continue;
    if (c.direction === "NEUTRAL") { neutral++; continue; }

    const dirReturn = c.direction === "LONG" ? ret : -ret;
    const netReturn = dirReturn - costPct / 100;
    trades.push({ i, direction: c.direction, confidence: c.confidence, consensus: c.consensus, dirReturn, netReturn, win: netReturn > 0 });
    if (!overlapping) nextAvailable = i + horizon;
  }

  return { trades, neutral, o, memberStats, cfg: { horizon, warmup, timeframe, overlapping, costPct } };
}

async function backtestCouncil(symbol, opts = {}) {
  const g = await generateTrades(symbol, opts);
  if (g.error) return g;
  return summarize(g.trades, g.neutral, g.o, g.cfg, g.memberStats);
}
...
module.exports = { backtestCouncil, generateTrades };
